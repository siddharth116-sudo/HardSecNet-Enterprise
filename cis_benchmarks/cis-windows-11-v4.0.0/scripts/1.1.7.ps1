...
if ($Status -or (-not $Apply -and -not $Rollback)) {
    ...
    if ($current -eq $DesiredValue) {
        Write-Output "Compliant: Reversible password encryption is disabled."
        exit 0
    }
    Write-Output ("Not compliant: ClearTextPassword is {0} (expected {1})." -f (Show-Value $currentRaw), $DesiredValue)
    exit 2
}
...
Write-Output "Applied: Reversible password encryption disabled (backup saved to $backup)."
