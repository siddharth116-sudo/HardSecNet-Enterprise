...
if ($Status -or (-not $Apply -and -not $Rollback)) {
    ...
    if ($current -eq $DesiredValue) {
        Write-Output "Compliant: Password complexity requirements are enabled."
        exit 0
    }
    Write-Output ("Not compliant: Password complexity is {0} (expected {1})." -f (Show-Value $currentRaw), $DesiredValue)
    exit 2
}
...
Write-Output "Applied: Password complexity enabled (backup saved to $backup)."
