...
# Control: 1.2.4 - Ensure 'Reset account lockout counter after' is set to '15 or more minute(s)'
# Source Page: 64
# Confidence: 0.96
# Status: ready
...
Write-StatusLine -Label 'Desired' -Value ("{0} or more (minutes)" -f $DesiredValue)

if ($Status -or (-not $Apply -and -not $Rollback)) {
    $current = if ($null -ne $currentRaw) { [int]$currentRaw } else { -1 }
    if ($current -ge $DesiredValue) {
        Write-Output "Compliant: Reset account lockout counter after is $current minute(s)."
        exit 0
    }
    Write-Output ("Not compliant: Reset lockout counter is {0} (expected >= {1})." -f (Show-Value $currentRaw), $DesiredValue)
    exit 2
}
...
Write-Output "Applied: Reset lockout counter set to $DesiredValue minute(s) (backup saved to $backup)."
