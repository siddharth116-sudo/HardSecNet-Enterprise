...
if ($Status -or (-not $Apply -and -not $Rollback)) {
    ...
    if ($current -ge $DesiredValue) {
        Write-Output "Compliant: Account lockout duration is $current minute(s)."
        exit 0
    }
    Write-Output ("Not compliant: Account lockout duration is {0} (expected >= {1})." -f (Show-Value $currentRaw), $DesiredValue)
    exit 2
}
...
Write-Output "Applied: Account lockout duration set to $DesiredValue minute(s) (backup saved to $backup)."
