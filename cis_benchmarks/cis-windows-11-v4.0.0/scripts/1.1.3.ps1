...
if ($Status -or (-not $Apply -and -not $Rollback)) {
    ...
    if ($current -ge $DesiredValue) {
        Write-Output "Compliant: Minimum password age is $current day(s)."
        exit 0
    }
    Write-Output ("Not compliant: Minimum password age is {0} (expected >= {1})." -f (Show-Value $currentRaw), $DesiredValue)
    exit 2
}
...
Write-Output "Applied: Minimum password age set to $DesiredValue day(s) (backup saved to $backup)."
