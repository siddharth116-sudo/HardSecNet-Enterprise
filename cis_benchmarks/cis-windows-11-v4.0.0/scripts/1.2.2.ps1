...
# Control: 1.2.2 - Ensure 'Account lockout threshold' is set to '5 or fewer invalid logon attempt(s), but not 0'
# Source Page: 60
# Confidence: 0.96
# Status: ready
...
$currentRaw = Get-SecPolValue -CfgPath $tmp -Key $SettingKey

Write-StatusLine -Label 'Control' -Value $ControlId
Write-StatusLine -Label 'Setting' -Value $SettingKey
Write-StatusLine -Label 'Current' -Value (Show-Value $currentRaw)
Write-StatusLine -Label 'Desired' -Value ("1..{0} (not 0)" -f $DesiredMax)

if ($Status -or (-not $Apply -and -not $Rollback)) {
    $current = if ($null -ne $currentRaw) { [int]$currentRaw } else { -1 }
    if ($current -ge 1 -and $current -le $DesiredMax) {
        Write-Output "Compliant: Account lockout threshold is $current invalid attempt(s)."
        exit 0
    }
    Write-Output ("Not compliant: Account lockout threshold is {0} (expected 1..{1})." -f (Show-Value $currentRaw), $DesiredMax)
    exit 2
}
...
Set-SecPolValue -CfgPath $tmp -Key $SettingKey -Value ([string]$DesiredMax)
Import-SecurityPolicy -CfgPath $tmp
Write-Output "Applied: Account lockout threshold set to $DesiredMax (backup saved to $backup)."
